...
const String myKey =
    'ca7609f6-fd29-4a43-b592-af7b36fb80b6'; //"hardcoded" so the same list of todos appear every time the app starts
...
Future<List<Todo>> getTodos() async {
  ...
  List todosJson = jsonDecode(
    response.body,
  ); //the id for each todo is given from the API
...
Future<List<dynamic>> postTodo(String title) async {
  final response = await http.post(
    ...
    body: jsonEncode({
      "title": title,
      "done": false,
    }), //only title and done is submitted, not id
  );
...
Future<void> putTodo(String id, bool done, String title) async {
  final response = await http.put(
    Uri.parse('$ENDPOINT/todos/$id?key=$myKey'),
    headers: {"Content-Type": "application/json"},
